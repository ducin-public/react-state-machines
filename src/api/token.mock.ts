...
export const tokenMockHandlers = [
  rest.post<EMPTY_STRING, {}, TokenInstruction>(`${API_URL}/banking/token`, async (req, res, ctx) => {
    return res(
      ctx.delay(delayTime()),
      ctx.json({
  ...
  rest.post<TokenConfirmation, { tokenId: string }, EMPTY_STRING>(`${API_URL}/banking/token/:tokenId`, async (req, res, ctx) => {
