...
export const getEmployees = async (page = 1) => {
  const response = await axios.get<Employee[]>(`${API_URL}/employees?_page=${page}`)
...
export const updateEmployee = async ({ id, ...changes }: EmployeeChanges) => {
  const response = await axios.patch<void>(`${API_URL}/employees/${id}`, changes)
  return response.data
}

export const deleteEmployee = async (id: Employee['id']) => {
  const response = await axios.delete<void>(`${API_URL}/employees/${id}`)
  return response.data
}
