...
export const changeLimitsVerificationMethodLabel: { [key in VerificationMethod] : string } = {
  "SMS-CODE": "SMS code",
  "SCRATCH-CARD-CODE": "scratch card",
}
...
export const getLimits = async () => {
  const response = await axios.get<LimitsSettings>(`${API_URL}/banking/limits`);
...
export const sendLimitsUpdate = async (payload: SendLimitsUpdatePayload) => {
  const response = await axios.post<SendLimitsUpdateResponse>(`${API_URL}/banking/limits`, payload);
  return response.data;
}
